...
pthread_t threads[MAX_CONN]; //list of worker threads
...
char filecontent_buffers[MAX_CONN][CONTENT_SIZE]; //list of file content buffers for each thread
...
int thread_pipes[MAX_CONN][2]; //communication pipes for each thread
...
void report_peer_connected(const struct sockaddr_in* sa, socklen_t salen) {
  char hostbuf[256];
  char portbuf[256];
  if (getnameinfo((struct sockaddr*)sa, salen, hostbuf, 256, portbuf,
                  256, 0) == 0) {
...
void * worker_function(int *arg){
    ...
    while(1){
      ...
      printf("Thread id is %d", *arg);
      printf("WORKER FUNCTION WAITING...");
			...
			peerstate->state = IN_MSG;
		      ...
		      //break;
				...
				puts("Received message from client\n");
     				puts(filename_buffers[peerstate->worker_thread_id]);
     				puts("WORKER CALLED");
       
  				ready_to_send = true; //Message ended, now we can send

				//Copy the message to the filename buffers
				printf("ID OF THREAD %d\n", peerstate->worker_thread_id);
				//strcpy(filename_buffers[peerstate->worker_thread_id], buf); 
				printf("SENDING MSG %s", filename_buffers[peerstate->worker_thread_id]);
				...
				printf("WORKER FUNCTION SIGNALED...");
				...
				return fd_status_W;
                  		...
                  		//puts("APPENDING TO READ BUFFER");
				int index = peerstate->worker_thread_id;
				filename_buffers[index][filename_ptr_ends[index]] = buf[i];
				printf("FILENAME BUFFER %s", filename_buffers[index]);
				printf("FILENAME  PTR %d", filename_ptr_ends[index] );
  	...
  	if(peerstate-> filename_read == 0){
		...
		char buf[CONTENT_SIZE];
  		...
  		if (nsent < sendlen) {
        		...
        		puts("Message successfully sent to client");
    			puts(filecontent_buffers[peerstate->worker_thread_id]);
			...
			if (peerstate->state == INITIAL_ACK) {
			  peerstate->state = WAIT_FOR_MSG;
   			 ...
   			 puts("All client operations are handled, client can quit");
  ...
  while (1) {
    ...
    if (epoll_ctl(epollfd, EPOLL_CTL_ADD, newsockfd, &event) < 0) {
        ...
        if (events[i].events & EPOLLIN) {
          // Ready for reading.
          //puts("Now server is ready for receiving");
          ...
          //puts("Now server is ready for sending");
          // Ready for writing.
          int fd = events[i].data.fd;
